...
Please refer to:
    ...
    https://ieeexplore.ieee.org/document/9627039
"""
import numpy as np
...
class PrincipalAxesKalmanFilter(AbtractEllipticalTracker):
    ...
    def __init__(self,
        ...
        self.p = np.array(p_init).astype(float)
    ...
    def update(self, measurements: np.ndarray, fix_orientation: float = None):
        ...
        # catch to small evals
        eig_vals[eig_vals < 1e-2] = 1e-2
        ...
        major_ev = Q[:, major_ix]
        theta = np.arctan2(major_ev[1], major_ev[0]) % (self.f * np.pi)
        ...
        innov_p[0] = self._orientation_innovation(x=y_hat_p[0], z=y_p[0], f=self.f)
        ...
        p_plus[0] = p_plus[0] % (self.f * np.pi)
    ...
    @staticmethod
    def _orientation_innovation(x, z, f=2):
        """
        Calculate the innovation between a state x and a measurement z for a circular state space in 0,f*pi

        Changes z in order to deal with "wrap-around" cases at 0 and f*pi
        :param x: State
        :param z: Measurement
        :return: Innovation between parameters
        """
        if np.abs(x - (z + f * np.pi)) < np.abs(x - z):
            z += f * np.pi
        elif np.abs(x - (z - f * np.pi)) < np.abs(x - z):
            z -= f * np.pi
        return z - x
